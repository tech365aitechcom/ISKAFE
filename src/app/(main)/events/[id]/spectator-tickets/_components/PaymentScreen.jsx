...
const PaymentScreen = ({ eventDetails, onNext, onBack, onCancel, purchaseData }) => {
  ...
  const processSquarePayment = async () => {
    ...
    const paymentData = {
      note: `Spectator ticket purchase - Event: ${eventDetails?.name || 'Unknown Event'}`
  ...
  const processPurchase = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'cash') {
        ...
        purchasePayload.transactionId = transactionId
        console.log('Square payment completed, transaction ID:', transactionId)
      }

      console.log('Final purchase payload before API call:', JSON.stringify(purchasePayload, null, 2))
      
      // Double check all required fields are present for the backend API
      const requiredFields = ['eventId', 'tierName', 'quantity', 'buyerType', 'paymentMethod']
      const missingFields = requiredFields.filter(field => !purchasePayload[field])
      ...
      if (purchasePayload.buyerType === 'guest' && (!purchasePayload.guestDetails || !purchasePayload.guestDetails.email)) {
        console.error('Guest details with email are required for guest buyer type')
        throw new Error('Guest email is required for guest purchases')
      }

      const response = await axios.post(`/spectator-ticket/purchase`, purchasePayload)
  ...
  const handlePay = () => {
    ...
    if (paymentMethod === 'card') {
      ...
      if (!squareConfigValid) {
        enqueueSnackbar('Payment system not configured. Please try cash payment or contact support.', { variant: 'error' })
        return
      }
      ...
      if (!squareLoaded) {
        enqueueSnackbar('Payment system is still loading. Please wait.', { variant: 'warning' })
        return
      }
      
      if (!cardInstance.current) {
        enqueueSnackbar('Payment form not ready. Please wait and try again.', { variant: 'warning' })
        return
      }
      ...
      <div className="bg-[#0A1330] rounded-lg p-6 mb-8">
        ...
        {purchaseData.tickets.map((ticket, index) => (
          <div key={index} className="flex justify-between items-center mb-2">
            ...
            <span className="font-bold">${(ticket.price * ticket.quantity).toFixed(2)}</span>
          </div>
        ))}
        ...
        <div className="mb-8">
          ...
          {squareConfigValid && squareLoaded && (
            <div className="bg-[#0A1330] rounded-lg p-6">
              <div id="square-card-container" ref={cardRef} className="mb-4" />
              {errors.square && (
