...
export default function SuspensionModal({ fighter, onClose, onSave }) {
  ...
  useEffect(() => {
    ...
    if (formData.suspensionStartDate && formData.suspensionDuration) {
      ...
      endDate.setDate(startDate.getDate() + parseInt(formData.suspensionDuration))
      ...
      <div className="bg-[#0B1739] rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-white">
            Add Suspension - {fighter.userId.firstName} {fighter.userId.lastName}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor="medicalClearanceRequired" className="text-white">
          ...
          <div className="p-4 bg-[#07091D] rounded-lg">
            <h3 className="text-sm font-medium text-white mb-2">Fighter Information</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-gray-400">Name:</span>
                <span className="ml-2 text-white">
                  {fighter.userId.firstName} {fighter.userId.lastName}
                </span>
              </div>
              <div>
                <span className="text-gray-400">Email:</span>
                <span className="ml-2 text-white">{fighter.userId.email}</span>
              </div>
              <div>
                <span className="text-gray-400">Weight Class:</span>
                <span className="ml-2 text-white">{fighter.weightClass || 'N/A'}</span>
              </div>
              <div>
                <span className="text-gray-400">Training Experience:</span>
                <span className="ml-2 text-white">{fighter.trainingExperience || 'N/A'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-4 pt-4 border-t border-gray-600">
            ...
            <button
              ...
              disabled={loading || !formData.suspensionType || !formData.suspensionDuration || !formData.suspensionStartDate}
            >
